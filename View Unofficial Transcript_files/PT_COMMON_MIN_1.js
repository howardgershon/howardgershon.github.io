...
PT_Dialog.prototype = {
 ...
 MTop().oParentWin = null; MTop().modId = -1; if ((typeof modObj.bRCFModal != 'undefined' && !modObj.bRCFModal) && (typeof window.modWin == 'undefined'))
...
function closeLastModal(evt, target, oParentWin) {
 ...
 var histElement; var histElementString = sessionStorage.getItem("pt_doback_popped_element_nui"); if (typeof histElementString != undefined && histElementString && target && (target.parentElement.id == "#ALERTYES" || target.parentElement.id == "#ICYes$span")) {
...
function getPortalNodePart(sUrl){
 if (!isPIAUrl(sUrl) && !isPortalUrl(sUrl)) return null; var urlSplit = String(sUrl).match(/\/ps(c|p)\/?([^\/]*)?\/?([^\/]*)?\/?([^\/]*)?\//); if (urlSplit.length > 4)
...
function convToHostURI(sUrl, hostWin) {
 ...
 return sUrl; var sHostURI = getUrlHost(sHostUrl); var sHostPSHome = getPSHome(sHostUrl); var sPSHomeSuffix = getPSHomeSuffix(sUrl); var sHostPortalName = getPortalName(sHostUrl);  var sNodeName = getNodeName(sUrl) + "/"
 var pos = sUrl.indexOf(sNodeName); var sReturnUrl = sHostURI + '/psp/' + sHostPSHome+sPSHomeSuffix + '/' + sHostPortalName + '/' + sUrl.substring(pos, sUrl.length); return sReturnUrl;}
...
var backNavigation = {
 ...
 backBtnTxt.innerHTML = backRec.label; backNavigation.setAltText(objBackImg); backNavigation.setTitle(objBackImg.parentNode.parentNode); backNavigation.setAriaLabel(objBackImg.parentNode, backRec.label); removeClass(backBtn.parentNode.parentNode, "psc_disabled");  } else {
